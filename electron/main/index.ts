...
import { ensurerunjianClawContext, repairrunjianClawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.runjianClaw.desktop';
...
// On Wayland this maps the running window to runjianClaw.desktop (→ icon + app grouping);
// on X11 it supplements the StartupWMClass matching.
// Must be called before app.whenReady() / before any window is created.
if (process.platform === 'linux') {
  app.setDesktopName('runjianClaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[runjianClaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[runjianClaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[runjianClaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== runjianClaw Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain runjianClaw markers (no OpenClaw
  // template content). This fixes a race condition where ensurerunjianClawContext()
  // previously created the file before the gateway could seed the full template.
  void repairrunjianClawOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running') {
      void ensurerunjianClawContext().catch((error) => {
        logger.warn('Failed to re-merge runjianClaw context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge runjianClaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensurerunjianClawContext will retry until the target files appear.
  void ensurerunjianClawContext().catch((error) => {
    logger.warn('Failed to merge runjianClaw context into workspace:', error);
  });
...
if (gotTheLock) {
  ...
  app.on('second-instance', () => {
    logger.info('Second runjianClaw instance detected; redirecting to the existing window');
