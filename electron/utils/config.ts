 ...
 */
export const PORTS = {
  /** runjianClaw GUI development server port */
  runjianClaw_DEV: 5173,
  
  /** runjianClaw GUI production port (for reference) */
  runjianClaw_GUI: 23333,

  /** Local host API server port */
  runjianClaw_HOST_API: 3210,
...
export function getPort(key: keyof typeof PORTS): number {
  const envKey = `runjianClaw_PORT_${key}`;
...
export const APP_PATHS = {
  ...
  /** runjianClaw configuration directory */
  runjianClaw_CONFIG: '~/.runjianClaw',
  
  /** Log files directory */
  LOGS: '~/.runjianClaw/logs',
} as const;
