...
interface SyncProxyOptions {
   ...
   * currently disabled in runjianClaw settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync runjianClaw global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because runjianClaw proxy is disabled and preserve mode is enabled');
      return;
    }
