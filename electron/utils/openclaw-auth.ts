 ...
 * Used by runjianClaw to seed the provider store when it's empty but providers are
...
/**
 * Write the runjianClaw gateway token into ~/.openclaw/openclaw.json.
 */
export async function syncGatewayTokenToConfig(token: string): Promise<void> {
  return withConfigLock(async () => {
    ...
    // Packaged runjianClaw loads the renderer from file://, so the gateway must allow
 ...
 * which means conversations disappear after roughly one day.  runjianClaw sets
 ...
 * OpenClaw/runjianClaw versions or external tools.
 *
...
export async function sanitizeOpenClawConfig(): Promise<void> {
  return withConfigLock(async () => {
    ...
    // When runjianClaw-managed moonshot provider exists, prefer centralized key
    ...
    // for runjianClaw to properly integrate with its updated tool system.
    ...
    // but runjianClaw historically stored them only under `channels.<type>.accounts.default`.
