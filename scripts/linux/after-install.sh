...
# Post-installation script for runjianClaw on Linux
...
# Create symbolic link for runjianClaw app binary
if [ -x /opt/runjianClaw/runjianClaw ]; then
    ln -sf /opt/runjianClaw/runjianClaw /usr/local/bin/runjianClaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/runjianClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/runjianClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/runjianClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/runjianClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/runjianClaw'
...
echo "runjianClaw has been installed successfully."
